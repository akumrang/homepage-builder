...
다음 1순위 추천 작업은 로컬 개발 DB 정리 절차 문서 변경분을 커밋한 뒤 모바일/데스크톱 브라우저 스크린샷 기반 자동 회귀 테스트를 추가하는 것이다.

이유:

- 시각 QA는 이미 수동으로 1차와 2차를 진행했다.
- 출시 전 같은 화면을 반복 확인하려면 브라우저 기반 캡처 자동화가 필요하다.
