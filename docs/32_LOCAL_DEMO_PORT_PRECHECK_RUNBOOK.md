...
| Vite가 자동으로 `5176` 등 대체 포트로 밀렸는데 backend CORS를 맞추지 않음 | HOLD |
...
`5175`가 다른 workspace에서 이미 사용 중이고 그 프로세스를 종료하지 않을 경우, homepage frontend는 Vite에 의해 `5176` 같은 대체 포트로 실행될 수 있다.
...
이때 `5175` 문서 기본 URL과 실제 검증 URL이 다르다는 사실을 보고서에 남긴다.
...
커밋 후 추천 작업은 `demo:ports` 결과를 README의 실행 순서에 더 강하게 반영하고, 묵산 시각 판정 전 안내 문구를 대화창 기준으로 정리하는 것이다.
