...
서비스 wrapper XML 초안은 `deploy/windows/muksan-homepage-backend.winsw.xml.template`을 기준으로 한다.
실제 service install/start/stop/restart 명령 순서는 `docs/09_WINDOWS_OPERATION_REHEARSAL_CHECKLIST.md`를 따른다.
...
- Caddyfile 초안은 `deploy/windows/Caddyfile.template`을 기준으로 한다.
- Caddy validation과 run/reload 확인은 `docs/09_WINDOWS_OPERATION_REHEARSAL_CHECKLIST.md`를 따른다.
