...
- backend 시작 후 `/api/health`가 정상 응답한다.
...
| `VITE_API_BASE_URL` | 배포 방식에 따라 필요 | frontend와 backend를 분리 호스팅할 때 frontend build 전에 지정 |
...
현재 backend CORS는 로컬 개발 origin만 허용한다. 실제 도메인 배포는 다음 둘 중 하나로 처리한다.

- 권장: frontend와 backend를 같은 origin으로 reverse proxy 구성
- 별도 origin: 허용 origin을 환경 변수로 관리하도록 backend CORS 설정을 추가 구현
...
- 현재 `/api/health`는 프로세스 생존 확인이다.
- DB 연결, migration 상태, seed 상태까지 확인하는 readiness endpoint는 아직 없다.
- 외부 load balancer의 기본 health check는 `/api/health`를 사용하되, 운영 안정화 단계에서는 `/api/ready` 또는 동등한 readiness check를 별도 구현한다.
