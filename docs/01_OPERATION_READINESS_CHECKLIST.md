...
Windows 운영 배포 첫 실행 순서는 `docs/10_WINDOWS_DEPLOYMENT_QUICKSTART.md`를 따른다.
...
MVP 1차 운영 환경 결정은 `docs/07_MVP_PRODUCTION_ENVIRONMENT_DECISION.md`를 따른다.
Windows Caddy와 Service wrapper 템플릿은 `docs/08_WINDOWS_CADDY_SERVICE_TEMPLATES.md`를 따른다.
...
- SQLite DB, backup, log는 `C:/muksan-homepage/*` 아래 checkout 밖 보관
...
- 실제 고객 게시라면 `docs/12_PUBLICATION_MODE_POLICY.md`의 게시 모드 기준 확인
- 공개 홈페이지 첫 화면 접속
...
- 고객 게시 화면에 샘플 고지, 샘플 주소, 샘플 전화번호, 내부 링크가 남아 있지 않은지 확인
...
| MVP 1차 운영 환경 결정 | `docs/07_MVP_PRODUCTION_ENVIRONMENT_DECISION.md` |
| Windows Caddy/Service 템플릿 | `docs/08_WINDOWS_CADDY_SERVICE_TEMPLATES.md`, `deploy/windows/` |
| Windows 운영 수동 리허설 | `docs/09_WINDOWS_OPERATION_REHEARSAL_CHECKLIST.md` |
| Windows 배포 Quickstart | `docs/10_WINDOWS_DEPLOYMENT_QUICKSTART.md` |
| Windows runtime 사전점검 | `deploy/windows/Test-MuksanHomepageRuntime.ps1` |
| Windows service 조작 보조 스크립트 | `deploy/windows/Invoke-MuksanHomepageService.ps1` |
| Windows Caddy 조작 보조 스크립트 | `deploy/windows/Invoke-MuksanHomepageCaddy.ps1` |
