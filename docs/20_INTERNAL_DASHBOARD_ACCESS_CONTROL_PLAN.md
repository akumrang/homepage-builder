...
- 로컬 개발 DB의 테스트 문의와 제작 상태값을 시연 전 어떻게 정리할지 문서화한다.
- 실제 운영 DB 삭제나 destructive reset 절차는 만들지 않는다.
- 고객 포털, 고객 계정, 자유 편집기는 계속 보류한다.

이 작업은 고객 기능을 늘리는 것이 아니라, 묵산이 파일럿 시연 전 내부 화면을 통제된 상태로 확인하게 만드는 작업이다.
