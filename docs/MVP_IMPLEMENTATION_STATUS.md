...
커밋 후 다음 추천 작업은 샘플 갤러리 기획 재검토다. 이 작업은 구현이 아니라 고객 디자인 권리 2단계인 `방향 선택형`의 범위, 기대 관리 문구, 선택값 처리 방식을 정리하는 단계다.
