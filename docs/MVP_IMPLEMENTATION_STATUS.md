...
커밋 후 다음 추천 작업은 샘플 카드 문안과 고객 기대 관리 문구 초안 작성이다. 이 작업도 구현이 아니라 고객에게 보여줄 제목, 설명, 적합/부적합 안내, 선택 전 고지 문구를 문서로 정리하는 단계다.
