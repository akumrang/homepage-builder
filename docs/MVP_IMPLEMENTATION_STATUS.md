...
내부 식별자 rename 정책은 `KEEP`으로 판정했다. 고객 노출 문구는 `베틀 시스템`으로 관리하고, 내부 codename과 기술 식별자는 당장 바꾸지 않는다.

가상 파일럿 발송 리허설은 통과했다. 실제 고객 발송이 아니라, 가상 학원 정보를 넣어 발송 문안과 내부 범위 분리가 자연스럽게 작동하는지 확인했다.

가상 고객 회신 접수 리허설 재점검은 통과했다.

파일럿 시연 URL 준비 상태 점검은 완료했다. 현재 로컬 내부 시연 URL은 준비되어 있으나, 외부 공유 URL은 아직 없다.

로컬 시연 URL 브라우저 확인은 완료했다. homepage 화면 자체는 `5176` 대체 포트에서 통과했지만, 현재 `5175`는 academy frontend가 점유하고 있어 문서상 기본 URL은 시연 URL로 HOLD다.

로컬 시연 포트 충돌 사전점검 절차를 추가했다. `npm.cmd run demo:ports`가 `5175`, `4200` 점유 프로세스가 homepage workspace인지 확인하고, 다른 workspace가 점유 중이면 HOLD로 실패한다.

로컬 시연 실행 순서와 묵산 시각 판정 전 대화창 안내 문구를 README에 정리했다. 시각 판정 요청은 실제 URL, 판정 대상, 판정하지 않을 항목, 포트 상태, `GO/HOLD/수정 요청` 응답 기준을 이 대화창에서 명시하는 방식으로 고정했다.

로컬 시연 포트 정책은 `5175` 유지로 결정했다. `5176`은 공식 로컬 시연 URL로 승격하지 않고 임시 검증용으로만 허용한다. frontend dev/preview는 strict port로 실행해 `5175` 충돌 시 자동 대체 포트로 이동하지 않게 했다.

묵산은 2026-07-01 이 대화창에서 `go. 다음`으로 공개 샘플 홈페이지 시각 방향을 GO 판정했다. 이 GO는 임시 로컬 검증 URL 기준의 MVP 공개 샘플 화면 승인으로 기록했고, 외부 공유 URL, 운영 배포, 실제 고객 발송 승인은 포함하지 않는다.

시각 GO 이후 첫 파일럿 전환 체크리스트를 정리했다. 현재 실제 고객은 없으므로 고객 발송 실행이 아니라 외부 공유 URL 방식, 자료 요청 패킷, 회신 처리, 운영 배포 준비를 분리해서 판단한다.

첫 파일럿용 외부 공유 URL 방식은 제한 공개 staging URL 우선으로 결정했다. 화면 공유는 보조 수단이고, 운영 기준 URL 바로 사용은 보류한다. 아직 실제 배포, 도메인 확정, 고객 발송은 하지 않는다.
